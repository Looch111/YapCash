...
async function updateAccountTokens(targetInfo, tokens = {}) {
  // CRITICAL CLOUD FIX: Ensure Cloud DB is fully hydrated from Firebase before processing token updates
  if (!firestoreAccountsCache || firestoreAccountsCache.size === 0) {
  ...
  if (index !== -1) {
    ...
    accounts[index].status = accounts[index].status || ACCOUNT_STATUS.PENDING;
