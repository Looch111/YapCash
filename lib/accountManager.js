...
function loadAccounts() {
  if (firestoreAccountsCache && firestoreAccountsCache.size > 0) {
    return Array.from(firestoreAccountsCache.values());
  }
...
function saveAccounts(accounts) {
  ...
  // Optional local atomic write for backup
...
function updateAccountTokens(targetInfo, tokens = {}) {
  ...
  if (index !== -1) {
    ...
    // SAFELY CREATE NEW ACCOUNT ENTRY (NEVER OVERWRITE account_1)
    const newNum = accounts.length + 1;
    matchedId = searchId || `account_${newNum}`;
    const newAcc = {
      accountId: matchedId,
      ...
      preferredBrand: "amazon",
    };
    accounts.push(newAcc);
    console.log(`✨ [AccountManager] Registered new account entry: ${matchedId} (${email || "N/A"})`);
  }
...
const BALANCES_FILE = path.join(__dirname, "../account_balances.json");

/**
 * Reads persistent account balances from account_balances.json.
 */
...
/**
 * Saves updated account balances to account_balances.json atomically.
 */
function saveAccountBalances(balances) {
  const tmpFile = `${BALANCES_FILE}.tmp`;
  try {
    fs.writeFileSync(tmpFile, JSON.stringify(balances, null, 2), "utf-8");
    fs.renameSync(tmpFile, BALANCES_FILE);
  } catch (err) {
...
/**
 * Updates a single account's persistent balance record.
 */
