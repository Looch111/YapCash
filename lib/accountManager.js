...
function updateAccountTokens(targetInfo, tokens = {}) {
  ...
  if (index !== -1) {
    ...
    // SAFELY CREATE NEW ACCOUNT ENTRY (ONLY IF TRULY UNKNOWN ACCOUNT)
    const newNum = accounts.length + 1;
    matchedId = (searchId && searchId !== "temp_sync") ? searchId : `account_${newNum}`;
    const orderNum = parseInt((matchedId || "").replace(/\D/g, ""), 10) || newNum;
    const newAcc = {
      accountId: matchedId,
      order: orderNum,
