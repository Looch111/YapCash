...
async function fetchAccountsFromFirestore() {
  try {
    const url = `${FIRESTORE_BASE_URL}/yapcash_accounts?pageSize=100&key=${FIREBASE_API_KEY}`;
...
function startFirestoreRealtimeListener(onAccountsUpdatedCallback, intervalMs = 2000) {
  setInterval(async () => {
    ...
    const currentSignature = JSON.stringify(freshAccounts.map(a => `${a.accountId}:${a.refreshToken}:${a.email}`));
