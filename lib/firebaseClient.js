  ...
  try {
    ...
    do {
      ...
      if (res && res.ok) {
        ...
        // If a request fails, stop pagination and rely on cached data if available
        break;
      }
    } while (pageToken);
    ...
    if (success) {
      ...
      // Only remove cached accounts if we got a verified valid response from Firestore
...
async function logPackHistoryToFirestore(historyData) {
  try {
    ...
    const payload = { fields: toFirestoreFields({ ...historyData, timestamp: new Date().toISOString() }) };
...
async function syncTelegramStateToFirestore(telegramState) {
  try {
    ...
    const payload = { fields: toFirestoreFields({ ...telegramState, updatedAt: new Date().toISOString() }) };
