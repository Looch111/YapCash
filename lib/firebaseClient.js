...
const DEFAULT_PROJECT_ID = process.env.FIREBASE_PROJECT_ID || "yapcash-autofarm-default";
...
async function fetchAccountsFromFirestore() {
  try {
    const url = `${FIRESTORE_BASE_URL}/yapcash_accounts`;
...
async function syncAccountToFirestore(accountData) {
  ...
  const merged = { ...existing, ...accountData, updatedAt: new Date().toISOString() };
