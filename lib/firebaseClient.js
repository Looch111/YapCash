...
function parseServiceAccountJson(inputStr) {
  ...
  // 2. Fix unescaped newlines/control characters in private_key string (Render multiline ENV fix)
  try {
    const fixedKey = raw.replace(/("private_key"\s*:\s*")([\s\S]*?)(")/g, (_, p1, p2, p3) => {
      const cleanedKey = p2.replace(/\r?\n/g, "\\n").replace(/\t/g, "\\t");
      return `${p1}${cleanedKey}${p3}`;
    });
    return JSON.parse(fixedKey);
  } catch (_) {}

  // 3. Fallback: Sanitize all unescaped control characters inside JSON strings
  try {
    ...
    console.warn("⚠️ [Firebase Admin SDK] Failed to parse Service Account JSON:", err.message);
    return null;
  }
}
