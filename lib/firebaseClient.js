...
const FIREBASE_CONFIG = {
  apiKey: process.env.FIREBASE_API_KEY || "AIzaSyCyQPZ3r0QLXOtbGcl44vhuVTh6eboL5Xk",
  authDomain: "login-and-sign-up-e39e4.firebaseapp.com",
  projectId: process.env.FIREBASE_PROJECT_ID || "login-and-sign-up-e39e4",
  storageBucket: "login-and-sign-up-e39e4.firebasestorage.app",
  messagingSenderId: "1025789251454",
  appId: "1:1025789251454:web:9e8716c25c9306077b6d2f",
};
...
function getAdminDb() {
  ...
  let envSa = process.env.FIREBASE_SERVICE_ACCOUNT;
  if (!envSa) {
    try {
      const candidates = fs.readdirSync(process.cwd()).filter((f) => f.includes("firebase-adminsdk") && f.endsWith(".json"));
