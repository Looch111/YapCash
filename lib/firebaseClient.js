...
const FETCH_CACHE_TTL_MS = 60000; // 60-second RAM cache TTL to prevent hitting Firebase 50k daily read limit

/**
 * Fetches all account documents from Firebase Firestore collection 'yapcash_accounts' with pagination.
 * Includes a 60s RAM cache TTL guard to eliminate redundant reads unless forceRefresh is true.
 */
async function fetchAccountsFromFirestore(forceRefresh = false) {
  const now = Date.now();
  if (!forceRefresh && (now - lastFetchTimestamp < FETCH_CACHE_TTL_MS) && firestoreAccountsCache && firestoreAccountsCache.size > 0) {
...
/**
 * Subscribes to real-time account updates via Firebase Admin SDK or smart low-frequency background polling (3 mins).
 */
function startFirestoreRealtimeListener(onAccountsUpdatedCallback, intervalMs = 180000) {
  const db = getAdminDb();
  if (db) {
  ...
  // REST API smart polling mode (3 minutes default = ~480 reads/day max out of 50k quota)
  setInterval(async () => {
    const freshAccounts = await fetchAccountsFromFirestore(true);
    const currentSignature = JSON.stringify(
      freshAccounts.map(a => `${a.accountId}:${a.refreshToken}:${a.email}:${a.status}:${a.proxy}:${a.totalXp}:${a.streak}:${a.lastRunAt}:${a.updatedAt}:${a.error}`)
    );
    ...
    if (currentSignature !== lastKnownAccountsSignature) {
      ...
      if (!isInitial) {
        console.log(`⚡ [Firebase Listener] Live Firestore Cloud DB update detected (${freshAccounts.length} accounts)!`);
