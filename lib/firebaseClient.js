      ...
      if (data.documents && Array.isArray(data.documents)) {
        ...
        data.documents.forEach((doc) => {
          ...
          if (accObj.accountId) {
            ...
            firestoreAccountsCache.set(accObj.accountId, accObj);
          }
        });
...
async function syncAccountToFirestore(accountData) {
  ...
  // Retrieve existing document from memory
  ...
  const merged = {
    ...
    accessToken: accountData.accessToken || existing.accessToken || "",
    email: accountData.email || existing.email || "",
    userId: accountData.userId || existing.userId || "",
    proxy: accountData.proxy || existing.proxy || "http://bmrtynfq:cd2hv07lt0yr@38.154.185.97:6370",
    preferredBrand: accountData.preferredBrand || existing.preferredBrand || "apple",
    totalXp: accountData.totalXp ?? existing.totalXp ?? 0,
    streak: accountData.streak ?? existing.streak ?? 0,
    lastActive: accountData.lastActive || existing.lastActive || new Date().toISOString().slice(0, 10),
    ...accountData,
  ...
  try {
    ...
    const payload = { fields: toFirestoreFields(merged) };
...
function startFirestoreRealtimeListener(onAccountsUpdatedCallback, intervalMs = 2500) {
  setInterval(async () => {
    ...
    if (freshAccounts.length > 0 && typeof onAccountsUpdatedCallback === "function") {
