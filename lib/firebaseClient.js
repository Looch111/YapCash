...
function getAdminDb() {
  ...
  try {
    ...
    if (envSa.trim().startsWith("{")) {
      saObj = JSON.parse(envSa);
    } else if (fs.existsSync(envSa)) {
      saObj = JSON.parse(fs.readFileSync(envSa, "utf-8"));
    } else {
      saObj = JSON.parse(Buffer.from(envSa, "base64").toString("utf-8"));
    }
...
async function fetchAccountsFromFirestore() {
  ...
  if (db) {
    try {
      ...
      console.warn("⚠️ Firebase Admin SDK fetch warning:", err.message);
    }
...
function startFirestoreRealtimeListener(onAccountsUpdatedCallback, intervalMs = 2000) {
  ...
  if (db) {
    ...
    db.collection("yapcash_accounts").onSnapshot(
      ...
      (err) => {
        console.warn("⚠️ [Firebase Admin Realtime Listener Warning]:", err.message);
      }
    );
    return;
  }
