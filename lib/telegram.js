...
async function executePackOpeningSequence(msgId, targetAccountId, packTier) {
  ...
  try {
    ...
    results.forEach(({ accountId, res }) => {
      ...
      } else if (res.ok && res.xpAwarded) {
        cardLines.push(`⚡ <b>${accountId}:</b> +${res.xpAwarded.toLocaleString()} XP Bonus!`);
      } else {
        cardLines.push(`⚠️ <b>${accountId}:</b> ${res.message || res.reason || "Failed"}`);
      }
    });
