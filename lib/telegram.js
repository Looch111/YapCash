...
async function handleCallbackQuery(callbackQuery) {
  ...
  if (cbData === "restart_server") {
    const getRandomDelay = (minMs = 500, maxMs = 900) => Math.floor(Math.random() * (maxMs - minMs + 1)) + minMs;

    const rebootFrames = [
      `🔄 <b>[1/10] Initiating Koyeb Cloud Reboot Protocol...</b>\n⏳ <i>Authenticating admin security tokens...</i>`,
      `🛡️ <b>[2/10] Securing Active Proxy Sockets...</b>\n🔒 <i>Releasing US & UK regional proxy connections...</i>`,
      `⚡ <b>[3/10] Pausing 14-Account Staggered Scheduler...</b>\n⏸️ <i>Freezing account execution queues...</i>`,
      `💾 <b>[4/10] Flushing Telemetry State & Supabase Session Logs...</b>\n📦 <i>Preserving 120s organic human session data...</i>`,
      `🔑 <b>[5/10] Verifying OAuth Refresh Token Integrity...</b>\n🟢 <i>All 14 account refresh tokens verified!</i>`,
      `🔥 <b>[6/10] Terminating Active NodeJS Worker Thread...</b>\n🛑 <i>Process SIGTERM signal dispatched...</i>`,
      `🔌 <b>[7/10] Unbinding Cloud API Listener...</b>\n🌐 <i>Port 8000 passive sync socket closed...</i>`,
      `🧱 <b>[8/10] Requesting Fresh Koyeb Container Allocation...</b>\n🚀 <i>Spinning up clean container image...</i>`,
      `🟢 <b>[9/10] Cloud Handshake Complete & State Sync Verified!</b>\n🧹 <i>Preparing clean Telegram UI interface...</i>`,
      `🗑️ <b>[10/10] Deleting Terminal Log & Triggering Container Boot...</b>\n✨ <i>Koyeb daemon booting up in ~2s...</i>`,
    ];
    ...
    for (let i = 0; i < rebootFrames.length; i++) {
      ...
      await new Promise((r) => setTimeout(r, getRandomDelay(500, 950)));
    }
  ...
  if (cbData === "show_status") {
    ...
    const lines = [
      `<b>📊 YAPCASH LIVE ACCOUNT STATUS OVERVIEW</b>`,
      `────────────────────────`,
    ];
    ...
    for (let i = 0; i < accounts.length; i++) {
      ...
      lines.push(`${flag} <code>${acc.accountId}</code> — <b>${xp}</b> (🟢 Active)`);
    }
