...
async function fetchAccountXpFast(acc, timeoutMs = 4000) {
  try {
    ...
    // On cold boot without cached data, allow up to 8s for proxy SSL handshakes
    ...
    const fetchPromise = (async () => {
      ...
      if (session) {
        ...
        if (userState) {
          ...
          const res = { xp: userState.total_xp, country, flag, streak: userState.current_streak };
    ...
    // Fallback to memory cache or disk DB if network timed out so N/A is NEVER shown unnecessarily
