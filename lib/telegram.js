...
async function handleCallbackQuery(callbackQuery) {
  ...
  if (cbData === "restart_server") {
    // Frame 1 (0.0s)
    await editTelegramMessageText(msgId, "🔄 <b>Restarting Koyeb Daemon Server...</b>\n⏳ <i>Initiating container reboot protocol...</i>", {
      reply_markup: { inline_keyboard: [] },
    }).catch(() => {});
    await new Promise((r) => setTimeout(r, 1200));

    // Frame 2 (1.2s)
    await editTelegramMessageText(msgId, "⚡ <b>Stopping 14-Account Farming Scheduler...</b>\n💾 <i>Flushing active session tokens & telemetry state...</i>", {
      reply_markup: { inline_keyboard: [] },
    }).catch(() => {});
    await new Promise((r) => setTimeout(r, 1200));

    // Frame 3 (2.4s)
    await editTelegramMessageText(msgId, "🔥 <b>Container Process Terminated!</b>\n🔌 <i>Requesting Koyeb Cloud Engine Reload...</i>", {
      reply_markup: { inline_keyboard: [] },
    }).catch(() => {});
    await new Promise((r) => setTimeout(r, 1200));

    // Frame 4 (3.6s)
    await editTelegramMessageText(msgId, "🟢 <b>Handshake Complete! Rebooting Container Now...</b>\n🗑️ <i>Cleaning Telegram Chat History...</i>", {
      reply_markup: { inline_keyboard: [] },
    }).catch(() => {});
    await new Promise((r) => setTimeout(r, 1400));

    // Step 5 (5.0s) -> Delete message & trigger Koyeb container reboot
    await deleteTelegramMessage(msgId).catch(() => {});
    process.exit(0);
    return;
  }
