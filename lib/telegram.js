...
async function handleCallbackQuery(callbackQuery) {
  ...
  if (cbData === "restart_server") {
    await deleteTelegramMessage(msgId).catch(() => {});
    const tmp = await sendTelegramMessage("🔄 <b>Restarting Koyeb Daemon Server...</b>\n⏳ <i>Killing active process & reloading container...</i>").catch(() => null);
    await new Promise((r) => setTimeout(r, 1500));
    if (tmp && tmp.data?.result?.message_id) {
      await deleteTelegramMessage(tmp.data.result.message_id).catch(() => {});
    }
    process.exit(0);
    return;
  }
