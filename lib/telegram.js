...
/**
 * Automatically dispatches or morphs the Server Boot Notification in-place when Koyeb daemon starts up.
 */
async function sendServerBootNotification() {
  ...
  // If a reboot message ID was saved prior to restart, edit it directly in place for 100% clean UI
  if (state.lastRebootMsgId) {
    const rebootMsgId = state.lastRebootMsgId;
    delete state.lastRebootMsgId;
    saveTelegramState(state);

    const editRes = await editTelegramMessageText(rebootMsgId, text, {
