...
/**
 * Dispatches live Proxy Server status report with IP, country, credentials, and stats.
 */
async function sendProxyInfoMessage() {
  try {
    const { getProxyInfo, startProxyServer, detectGeoLocation } = require("../proxyServer");
    startProxyServer(); // Ensure proxy server is active
    await detectGeoLocation().catch(() => {}); // Force resolution of public IP & country

    const info = getProxyInfo();
    const statusEmoji = info.running ? "🟢" : "🔴";
    const statusLabel = info.running ? "Active & Healthy" : "Offline";

    const locationText = info.country !== "Unknown"
      ? `${info.flag} <b>${info.country}</b>${info.city !== "Unknown" ? ` (${info.city})` : ""}`
      : "🌐 <i>Location detecting...</i>";

    const message = [
      `<b>🛡️ YAPCASH PROXY SERVER STATUS</b>`,
      `────────────────────────`,
      `${statusEmoji} <b>Status:</b> ${statusLabel}`,
      `🌍 <b>Location:</b> ${locationText}`,
      `🏢 <b>ISP/Host:</b> ${info.org}`,
      `🌐 <b>Public IP:</b> <code>${info.publicIp}</code>`,
      `🔌 <b>Port:</b> <code>${info.port}</code>`,
      `👤 <b>User:</b> <code>${info.user || "None"}</code>`,
      `🔐 <b>Password:</b> <code>${info.pass || "None"}</code>`,
      ``,
      `📋 <b>Copy-Paste Proxy URL:</b>`,
      `<code>${info.proxyUrl}</code>`,
      ``,
      `📊 <b>Requests Processed:</b> ${info.totalRequests.toLocaleString()} | <b>Uptime:</b> ${info.uptime}`,
      `────────────────────────`,
      `<i>🤖 Runs 2-in-1 with YapCash Daemon 24/7</i>`,
    ].join("\n");

    return sendTelegramMessage(message);
  } catch (err) {
    return sendTelegramMessage(`❌ Failed to retrieve proxy status: ${err.message}`);
  }
}
...
async function handleTextMessage(msg) {
  ...
  if (command === "/openpack" || command === "/pack" || command === "/openpack@bot" || command === "/pack@bot") {
    await sendAccountPickerMenu(msg);
  } else if (command === "/proxy" || command === "/proxy@bot" || command === "/getproxy" || command === "/getproxy@bot") {
    await sendProxyInfoMessage();
  } else if (command === "/help" || command === "/help@bot") {
    const helpText = [
      `<b>🤖 YAPCASH BOT COMMAND MANUAL</b>`,
      `────────────────────────`,
      `📦 <code>/openpack</code> - Interactive Reward Pack Opener`,
      `📦 <code>/pack</code>     - Quick Pack Opener Shortcut`,
      `🛡️ <code>/proxy</code>   - View Live Proxy Status & URL`,
      `ℹ️ <code>/help</code>     - Display this command manual`,
      `────────────────────────`,
    ].join("\n");
 ...
 */
async function fetchAccountXpFast(acc, timeoutMs = 800) {
  try {
    ...
    const fetchPromise = (async () => {
      ...
      if (session) {
        ...
        if (typeof userState?.total_xp === "number") {
          return userState.total_xp;
        }
      }
      return null;
    })();
...
async function sendAccountPickerMenu(msgOrId, isEdit = false) {
  ...
  // Fetch all account XP balances concurrently in parallel with a strict 800ms timeout cap
  const xpResults = await Promise.all(accounts.map((acc) => fetchAccountXpFast(acc, 800)));
  ...
  for (let i = 0; i < accounts.length; i++) {
    ...
    const xp = xpResults[i];
    const xpBadge = typeof xp === "number" ? ` (${xp.toLocaleString()} XP)` : "";

    row.push({
      text: `👤 ${acc.accountId}${xpBadge}`,
...
async function sendTierPickerMenu(msgId, targetAccountId) {
  ...
  const packTiers = [
    ...
    { id: "rare", name: "Rare Pack", cost: 1500 },
    { id: "elite", name: "Elite Pack", cost: 5000 },
  ];
...
async function handleCallbackQuery(callbackQuery) {
  ...
  // 6. Pack Tier selected -> execute animated unboxing
