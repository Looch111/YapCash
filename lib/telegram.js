...
let inMemoryTelegramState = null;
let activeLiveMessageId = null;
let activeLiveMessageType = "menu"; // "menu" or "overview"
let updateLiveDebounceTimer = null;

/**
 * Loads Telegram state (routine message IDs & gift card messages) from memory/Firestore/disk.
 */
function loadTelegramState() {
  if (inMemoryTelegramState) return inMemoryTelegramState;
  if (fs.existsSync(TELEGRAM_STATE_FILE)) {
    try {
      const raw = fs.readFileSync(TELEGRAM_STATE_FILE, "utf-8");
      inMemoryTelegramState = JSON.parse(raw);
      return inMemoryTelegramState;
    } catch (err) {
      // Fallback below
    }
  }
  inMemoryTelegramState = { routineMessageIds: [], giftCardMessages: {} };
  return inMemoryTelegramState;
}

/**
 * Async initialization of Telegram state from Firebase Firestore.
 */
async function initTelegramStateFromFirestore() {
  try {
    const { fetchTelegramStateFromFirestore } = require("./firebaseClient");
    const cloudState = await fetchTelegramStateFromFirestore();
    if (cloudState) {
      if (!cloudState.routineMessageIds) cloudState.routineMessageIds = [];
      if (!cloudState.giftCardMessages) cloudState.giftCardMessages = {};
      inMemoryTelegramState = cloudState;
      saveTelegramStateToDisk(cloudState);
      return cloudState;
    }
  } catch (_) {}
  return loadTelegramState();
}

function saveTelegramStateToDisk(state) {
  const tmpFile = `${TELEGRAM_STATE_FILE}.tmp`;
  try {
    fs.writeFileSync(tmpFile, JSON.stringify(state, null, 2), "utf-8");
    fs.renameSync(tmpFile, TELEGRAM_STATE_FILE);
  } catch (err) {
    console.error("Failed to save telegram state to disk:", err.message);
  }
}

/**
 * Saves Telegram state atomically to memory, disk, and Firebase Firestore.
 */
function saveTelegramState(state) {
  inMemoryTelegramState = state;
  saveTelegramStateToDisk(state);
  try {
    const { syncTelegramStateToFirestore } = require("./firebaseClient");
    syncTelegramStateToFirestore(state).catch(() => {});
  } catch (_) {}
}

async function saveTelegramStateAsync(state) {
  inMemoryTelegramState = state;
  saveTelegramStateToDisk(state);
  try {
    const { syncTelegramStateToFirestore } = require("./firebaseClient");
    return await syncTelegramStateToFirestore(state).catch(() => false);
  } catch (_) {
    return false;
  }
}
 ...
 * (Disabled per user request: Overview card button provides live status on demand).
 */
...
async function sendTokenUpdateNotification(accEntry) {
  ...
  // Trigger instant live UI update of the main menu card so the synced account leaves the expired list
  updateLiveTelegramMenu().catch(() => {});
 ...
 */
async function sendServerBootNotification(preloadedAccounts = null) {
  // Use pre-hydrated accounts if provided by daemon (avoids second Firestore fetch race showing 0)
  ...
  if (!accounts) {
    ...
    accounts = await fetchAccountsFromFirestore().catch(() => []);
  ...
  const { ACCOUNT_STATUS } = require("./accountManager");
  const failedAccounts = accounts.filter((a) => a.status === ACCOUNT_STATUS.FAILED);
  const workingCount = accounts.length - failedAccounts.length;

  const statusText = accounts.length === 0 
    ? "🟢 Online & Passive Listening" 
    : `🟢 Online (${workingCount} Active | ⚠️ ${failedAccounts.length} Expired)`;

  const countStr = accounts.length === 0 
    ? "0 Accounts (Waiting for Chrome sync)" 
    : (accounts.length === 1 ? "1 Account Connected" : `${accounts.length} Accounts Connected`);

  const lines = [
    `🚀 <b>YAPCASH CLOUD ENGINE ACTIVE</b>`,
    `🤖 <b>Daemon Status:</b> ${statusText}`,
    `🔥 <b>Cloud DB:</b> 🟢 Firebase Active`,
    `📊 <b>Accounts Loaded:</b> ${countStr}`,
  ];

  if (failedAccounts.length > 0) {
    lines.push(`────────────────────────`);
    lines.push(`🚨 <b>ATTENTION: EXPIRED TOKENS (${failedAccounts.length})</b>`);
    failedAccounts.forEach((acc) => {
      lines.push(`  • <code>${acc.accountId}</code> (${acc.email || "N/A"})`);
    });
    lines.push(`<i>💡 Log into PC Chrome with YapCash extension to hot-sync.</i>`);
  }

  lines.push(`────────────────────────`);
  lines.push(`⏰ <b>24-Hour Staggered Farming Active!</b>`);
  lines.push(`<i>Tap any button below for instant 1-tap control:</i>`);
  ...
  const bootMsgId = res?.result?.message_id || res?.message_id || res?.data?.result?.message_id;
  if (bootMsgId) {
    activeLiveMessageId = bootMsgId;
    activeLiveMessageType = "menu";
  }
  return res;
}

/**
 * Triggers an instant live UI edit of the active Telegram card when Firestore accounts change in real time.
 */
async function updateLiveTelegramMenu(providedAccounts = null) {
  if (!activeLiveMessageId) return;
  if (updateLiveDebounceTimer) clearTimeout(updateLiveDebounceTimer);

  updateLiveDebounceTimer = setTimeout(async () => {
    try {
      if (activeLiveMessageType === "overview") {
        await renderStatusOverviewMessage(activeLiveMessageId, providedAccounts);
      } else if (activeLiveMessageType === "menu") {
        await sendMasterControlMenu(activeLiveMessageId, true);
      }
    } catch (_) {}
  }, 300);
}
...
/**
 * Renders and edits the Status Overview message card live.
 */
async function renderStatusOverviewMessage(msgId, providedAccounts = null) {
  activeLiveMessageId = msgId;
  activeLiveMessageType = "overview";

  const { fetchAccountsFromFirestore } = require("./firebaseClient");
  const { loadAccounts } = require("./accountManager");
  let accounts = providedAccounts;
  if (!accounts) accounts = await fetchAccountsFromFirestore().catch(() => []);
  if (!accounts || accounts.length === 0) accounts = loadAccounts();
  ...
  const { ACCOUNT_STATUS } = require("./accountManager");
  let daemonState = { currentlyRunningAccountId: null, nextScheduledAccountId: null, nextScheduledTimeMs: null };
  try {
    const runner = require("../runner");
    if (runner && typeof runner.getLiveDaemonState === "function") {
      daemonState = runner.getLiveDaemonState();
    }
  } catch (_) {}
  ...
  for (let i = 0; i < accounts.length; i++) {
    ...
    const isRunningNow = acc.status === ACCOUNT_STATUS.ACTIVE || acc.accountId === daemonState.currentlyRunningAccountId;
    const isNextUp = acc.accountId === daemonState.nextScheduledAccountId;
    const isDoneToday = acc.status === ACCOUNT_STATUS.COMPLETED;

    if (isRunningNow) {
      lines.push(`🏃 <b><code>${acc.accountId}</code></b> │ ⚡ <b>FARMING NOW</b> │ ⚡ <b>${xpStr}</b>`);
    } else if (isNextUp) {
      let countdownStr = "soon";
      if (daemonState.nextScheduledTimeMs) {
        const remainingMs = Math.max(0, daemonState.nextScheduledTimeMs - Date.now());
        const mins = Math.floor(remainingMs / 60000);
        const secs = Math.floor((remainingMs % 60000) / 1000);
        countdownStr = mins > 0 ? `in ${mins}m ${secs}s` : `in ${secs}s`;
      }
      lines.push(`🎯 <b><code>${acc.accountId}</code></b> │ ⏳ <b>NEXT UP (${countdownStr})</b> │ ⚡ <b>${xpStr}</b>`);
    } else if (isDoneToday) {
      lines.push(`✅ <b><code>${acc.accountId}</code></b> │ ⚡ <b>${xpStr}</b> │ 🔥 <code>${streak}</code> │ ${packBadge}`);
    } else {
  ...
  lines.push(`<i>Updated: ${new Date().toISOString().replace("T", " ").slice(0, 19)} UTC (Real-Time Cloud Sync)</i>`);

  const res = await editTelegramMessageText(msgId, lines.join("\n"), {
    reply_markup: {
      inline_keyboard: [
        [
...
async function sendMasterControlMenu(msgOrId = null, isEdit = false) {
  const { fetchAccountsFromFirestore } = require("./firebaseClient");
  const { loadAccounts, ACCOUNT_STATUS } = require("./accountManager");
  ...
  const lines = [
    ...
    `────────────────────────`,
    `<b>Daemon State:</b> 🟢 Active (${workingCount} Ready | ⚠️ ${failedAccounts.length} Expired)`,
  ...
  lines.push(`────────────────────────`);
  lines.push(`Select an option below:`);
  ...
  const menuMsgId = res?.result?.message_id || res?.message_id || res?.data?.result?.message_id;
  if (menuMsgId) {
    activeLiveMessageId = menuMsgId;
    activeLiveMessageType = "menu";
  }
  return res;
}
...
async function fetchAccountXpFast(acc, timeoutMs = 4000) {
  ...
  const rawXp = acc && acc.totalXp !== undefined && acc.totalXp !== null ? Number(acc.totalXp) : NaN;
  if (acc && !isNaN(rawXp)) {
    const country = acc.rewardCountry || acc.country || "US";
    let flag = "🇺🇸";
    if (country === "GB" || country === "UK") flag = "🇬🇧";
    else if (country === "NG") flag = "🇳🇬";
    else if (country === "CA") flag = "🇨🇦";
    else if (country === "AU") flag = "🇦🇺";
    return {
      ...
      flag,
      streak: acc.streak ?? 0,
      lastActive: acc.lastRunAt || acc.lastActive || "Today",
      email: acc.email || "N/A",
...
async function sendAccountPickerMenu(msgOrId, isEdit = false) {
  ...
  if (accounts.length === 0) {
    return sendTelegramMessage("❌ No accounts registered yet. Please sync a token via the Chrome extension or the /api/sync-token endpoint.");
  }
...
async function sendTierPickerMenu(msgId, targetAccountId) {
  ...
  if (targetAccountId !== "ALL") {
    ...
    if (acc) {
      const info = await fetchAccountXpFast(acc, 800);
      totalXp = (info && typeof info.xp === "number") ? info.xp : null;
    }
  }
...
async function handleCallbackQuery(callbackQuery) {
  ...
  if (cbData === "show_status") {
    await renderStatusOverviewMessage(msgId);
    return;
  }
...
const tokenExpiredAlertDedupeMap = new Map();

/**
 * Sends a Telegram notification when an account token expires or authentication fails.
 * Deduplicated to avoid spamming the user more than once every 6 hours per account.
 */
async function sendTokenExpiredAlert(accountId, email = "N/A", reason = "Authentication token expired") {
  const now = Date.now();
  const lastSent = tokenExpiredAlertDedupeMap.get(accountId) || 0;

  // 6 hours deduplication interval per account
  if (now - lastSent < 6 * 60 * 60 * 1000) {
    return false;
  }

  tokenExpiredAlertDedupeMap.set(accountId, now);

  const text = [
    `🚨 <b>YAPCASH TOKEN EXPIRED ALERT!</b>`,
    `────────────────────────`,
    `👤 <b>Account:</b> <code>${accountId}</code>`,
    `📧 <b>Email:</b> <code>${email || "N/A"}</code>`,
    `⚠️ <b>Reason:</b> <code>${reason}</code>`,
    `────────────────────────`,
    `💡 <b>Action Required:</b> Please log into this account in PC Chrome with the YapCash extension to hot-sync fresh tokens automatically!`,
  ].join("\n");
...
module.exports = {
  loadTelegramConfig,
  loadTelegramState,
  initTelegramStateFromFirestore,
  saveTelegramState,
  sendTelegramMessage,
  ...
  updateLiveTelegramMenu,
  renderStatusOverviewMessage,
