...
const recentTokenSyncNotifications = new Map();
...
async function sendTokenUpdateNotification(accEntry) {
  const targetKey = accEntry.accountId || accEntry.email || "unknown";
  const now = Date.now();
  if (recentTokenSyncNotifications.has(targetKey) && (now - recentTokenSyncNotifications.get(targetKey)) < 3000) {
    return { ok: true, reason: "deduplicated" };
  }
  recentTokenSyncNotifications.set(targetKey, now);
  const text = [
