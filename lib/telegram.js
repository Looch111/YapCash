...
/**
 * Helper to fetch an account's live total_xp balance with cache fallback and cold-boot resilience.
 */
async function fetchAccountXpFast(acc, timeoutMs = 4000) {
  try {
    const SupabaseClient = require("./supabaseClient");
    const client = new SupabaseClient(acc);
    // On cold boot (empty cache), allow 8s for fresh proxy SSL handshakes
    ...
    const fetchPromise = (async () => {
      ...
      if (session) {
        ...
        if (userState) {
          ...
          const res = { xp: userState.total_xp, country, flag };
    ...
    // Fallback to cached state if network timed out so N/A is never shown unnecessarily
