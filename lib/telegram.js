...
async function sendDailyReport(reportData) {
  ...
  const accountSections = accounts.map((acc) => {
    if (acc.error) {
      return [
        `<b>▶️ ${acc.accountId}</b> (${acc.email || "N/A"})`,
        `  ❌ <i>Error: ${acc.error}</i>`,
      ].join("\n");
    }

    activeCount++;
    const startXp = acc.startXp ?? "N/A";
    const endXp = acc.endXp ?? "N/A";
    const gainedXp = acc.xpGained || 0;
    totalXpFarmed += gainedXp;

    const streak = acc.streak ?? "N/A";
    const bonusMsg = acc.bonusAwarded ? ` (+${acc.bonusAwarded} XP)` : "";
    const spinMsg = acc.spinAwarded ? ` (+${acc.spinAwarded} XP)` : "";

    let packStatus = "None";
    if (acc.packOpens && acc.packOpens.length > 0) {
      const wins = acc.packOpens.filter(p => p.ok && p.isWin);
      if (wins.length > 0) {
        totalWinsCount += wins.length;
        packStatus = wins.map(w => `🎉 <b>WINNER (${w.packId.toUpperCase()}):</b> ${w.brand} $${w.denom}`).join("\n    ");
      } else {
        packStatus = acc.packOpens.map(p => `${p.packId}: ${p.status}`).join(", ");
      }
    }

    return [
      `<b>▶️ ${acc.accountId}</b> (${acc.email})`,
      `  • <b>XP Progress:</b> ${startXp} ➔ <b>${endXp} XP</b> (+${gainedXp} XP farmed 🎯)`,
      `  • <b>Streak:</b> 🔥 ${streak} Days${bonusMsg}`,
      `  • <b>Wheel Spin:</b> 🎡${spinMsg || " Used"}`,
      `  • <b>Pack Status:</b> ${packStatus}`,
    ].join("\n");
  });

  const reportMessage = [
    `<b>📊 YAPCASH DAILY AUTOMATION REPORT</b>`,
    `📅 <b>Date:</b> ${dateStr} (UTC Reset Cycle)`,
    ``,
    accountSections.join("\n\n"),
    ``,
    `<b>📈 PORTFOLIO DAILY TOTALS:</b>`,
    `  • <b>Active Accounts Processed:</b> ${activeCount}/${accounts.length}`,
    `  • <b>Total XP Farmed Today:</b> <b>+${totalXpFarmed.toLocaleString()} XP</b>`,
    `  • <b>Total Gift Cards Won Today:</b> <b>${totalWinsCount}</b>`,
    `🤖 <i>YapCash Smart Daemon Mode running smoothly on autopilot</i>`,
  ].join("\n");
