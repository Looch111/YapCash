...
async function sendServerBootNotification(preloadedAccounts = null) {
  // Delete previous boot message on reload to keep chat 100% clean
  const state = loadTelegramState();
  if (state.lastBootMessageId) {
    await deleteTelegramMessage(state.lastBootMessageId).catch(() => {});
  }
  if (activeLiveMessageId) {
    await deleteTelegramMessage(activeLiveMessageId).catch(() => {});
  }
