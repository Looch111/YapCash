...
async function sendServerBootNotification() {
  ...
  const text = [
    ...
    `🤖 <b>Daemon Status:</b> Online & Active`,
    `🔥 <b>Cloud DB:</b> 🟢 Firebase Firestore Active`,
    `📊 <b>Accounts Loaded:</b> ${accounts.length}/14 Connected`,
    `⏰ <b>New 24-Hour Cycle Started at Slot 1!</b>`,
    `────────────────────────`,
...
async function sendMasterControlMenu(msgOrId = null, isEdit = false) {
  ...
  const text = [
    ...
    `────────────────────────`,
    `<b>Daemon State:</b> 🟢 Active & Staggered (14 Accounts)`,
...
async function executeSmartDrainSequence(msgId, targetAccountId) {
  ...
  try {
    ...
    // Pre-check XP balance in 0ms using RAM snapshot to eliminate unneeded network calls
    for (const acc of targetAccounts) {
      const cachedInfo = accountStateCacheMap.get(acc.accountId);
      const xpVal = acc.totalXp ?? (cachedInfo && typeof cachedInfo.xp === "number" ? cachedInfo.xp : null);
      if (xpVal !== null && xpVal < 500) {
        drainResults.push({
          accountId: acc.accountId,
          res: { totalOpens: 0, totalXpSpent: 0, opens: [], reason: "Insufficient XP (<500 XP)" },
        });
      } else {
    ...
    if (eligibleAccounts.length === 0 && drainResults.length > 0) {
      // All accounts have <500 XP - show instant summary in 0ms!
    } else {
      for (let idx = 0; idx < eligibleAccounts.length; idx++) {
        const acc = eligibleAccounts[idx];
        await editTelegramMessageText(
          msgId,
          `🌊 <i>Smart Drain Progress: Account [${idx + 1}/${eligibleAccounts.length}] <code>${acc.accountId}</code>... 💫</i>`,
          { reply_markup: { inline_keyboard: [] } }
        ).catch(() => {});

        const client = new SupabaseClient(acc);
        const drainPromise = drainAccountPacks(client, 10);
        const timeoutPromise = new Promise((resolve) =>
          setTimeout(() => resolve({ totalOpens: 0, totalXpSpent: 0, opens: [], reason: "Timeout (Proxy SSL)" }), 10000)
        );

        const res = await Promise.race([drainPromise, timeoutPromise]).catch((err) => ({
          totalOpens: 0,
          totalXpSpent: 0,
          opens: [],
    ...
    drainResults.forEach(({ accountId, res }) => {
      if (res && res.totalOpens > 0) {
        ...
        const reason = res && res.reason ? res.reason : "No packs opened (XP < 500 or limit)";
        cardLines.push(`ℹ️ <b>${accountId}:</b> ${reason}`);
      }
    });
    ...
    let outputText = cardLines.join("\n");
    if (outputText.length > 3500) {
      outputText = outputText.substring(0, 3400) + "\n...\n<i>(Report truncated to fit Telegram limit)</i>";
    }

    await editTelegramMessageText(msgId, outputText, {
      reply_markup: {
        inline_keyboard: [
          [
            { text: "🔙 Main Menu", callback_data: "main_menu" },
...
async function handleCallbackQuery(callbackQuery) {
  ...
  if (cbData === "show_status") {
    ...
    const lines = [
      `<b>📊 YAPCASH ENTERPRISE DASHBOARD</b>`,
      `🔥 <b>Cloud DB:</b> 🟢 Firebase Firestore Active`,
      `────────────────────────`,
    ];
    ...
    for (let i = 0; i < accounts.length; i++) {
      ...
      const email = info && info.email ? info.email : (acc.email || "N/A");
      const lastActive = info && info.lastActive ? info.lastActive : "Today";
      const proxyAddr = acc.proxy ? (acc.proxy.split("@")[1] || acc.proxy) : "Direct Server IP";

      if (info && typeof info.xp === "number") totalSystemXp += info.xp;
      if (acc.proxy) activeProxiesCount++;

      let packAffordability = "🔒 Need XP";
      if (xpVal >= 2000) packAffordability = "🎁 Elite Ready";
      else if (xpVal >= 1000) packAffordability = "🎁 Rare Ready";
      else if (xpVal >= 500) packAffordability = "🎁 Standard Ready";

      lines.push(`${flag} <b><code>${acc.accountId}</code></b> (<code>${email}</code>)`);
      lines.push(` ├ ⚡ <b>Balance:</b> <code>${xpStr}</code> | 🔥 <code>${streak}</code>`);
      lines.push(` ├ 🏷️ <b>Status:</b> ${packAffordability} | 📅 <code>${lastActive}</code>`);
      lines.push(` └ 🌐 <b>Proxy:</b> <code>${proxyAddr}</code> (🟢 Live)`);
      lines.push(``);
    }

    lines.push(`────────────────────────`);
    lines.push(`💎 <b>Total System XP:</b> <code>${totalSystemXp.toLocaleString()} XP</code>`);
    lines.push(`🌐 <b>Operational Proxies:</b> <code>${activeProxiesCount}/14 Active</code>`);
