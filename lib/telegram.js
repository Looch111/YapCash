  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (response.ok && data.ok) {
        ...
        if (msgId && inMemoryTelegramState) {
          inMemoryTelegramState.lastSentMessageId = msgId;
          saveTelegramStateToDisk(inMemoryTelegramState);
        }
...
async function sendServerBootNotification(preloadedAccounts = null) {
  // Ensure state is loaded from Firestore first
  const state = await initTelegramStateFromFirestore().catch(() => loadTelegramState());

  const idsToDelete = new Set([
    state?.lastBootMessageId,
    state?.lastSentMessageId,
    activeLiveMessageId,
  ].filter(Boolean));
  ...
  if (bootMsgId) {
    ...
    state.lastBootMessageId = bootMsgId;
    saveTelegramState(state);
  }
  return res;
}
...
async function renderStatusOverviewMessage(msgId, providedAccounts = null) {
  ...
  return await editTelegramMessageText(msgId, lines.join("\n"), {
    reply_markup: {
      inline_keyboard: [
        [
