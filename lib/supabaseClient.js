...
class SupabaseClient {
  ...
  async refreshSession() {
    ...
    // Note: token persistence is handled by the daemon loop after ensureAuthenticated() returns.
    // Do NOT call updateAccountTokens here — it causes duplicate Firestore writes per run.
  ...
  async ensureAuthenticated(forceRefresh = false) {
    if (forceRefresh || !this.isAccessTokenValid()) {
      try {
        ...
        if (!this.accessToken || !this.isAccessTokenValid()) throw err;
      }
    }
