...
async function drainAccountPacks(client, maxOpensPerRun = 10) {
  ...
  const summaryMsg = hitGiftCard
    ? `🏆 GIFT CARD WON! Drained ${opens.length} pack(s), spent ${totalXpSpent.toLocaleString()} XP.`
    : opens.length > 0
      ? `Drained ${opens.length} pack(s), spent ${totalXpSpent.toLocaleString()} XP.`
      : `No packs eligible to drain.`;
