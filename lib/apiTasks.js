...
async function claimDailyBonus(client) {
  ...
  try {
    ...
    const awarded = res?.awarded ?? res?.xp_awarded ?? 0;
    const streak = res?.current_streak ?? res?.streak ?? null;
    return {
      ok: true,
      awarded,
      totalXp: res?.total_xp ?? null,
      currentStreak: streak,
      message: `Daily bonus claimed: +${awarded} XP (Streak: ${streak ?? "N/A"})`,
    };
  } catch (err) {
    ...
    for (const card of unfulfilled) {
      ...
      try {
        ...
        if (res.ok) {
          recoveredCount++;
          recordWin({
            accountId: client.account?.accountId || "unknown",
            email: userEmail,
...
async function openRewardPack(client, packId = "standard") {
  ...
  try {
    ...
    // Flush session telemetry so account shows organic human activity prior to pack opening
    await flushSession(client, { activeSeconds: 120, messages: 1, chatXp: 50 }).catch(() => {});
...
async function drainAccountPacks(client, maxOpensPerRun = 10) {
  ...
  for (let i = 0; i < maxOpensPerRun; i++) {
    ...
    if (result.ok) {
      ...
      if (raw.includes("cooldown") || raw.includes("insufficient") || raw.includes("depleted") || raw.includes("country")) {
        break;
      }
    }
...
async function humanDelay(minSec = 5, maxSec = 20) {
  const delayMs = Math.floor(Math.random() * (maxSec - minSec + 1) * 1000) + (minSec * 1000);
  console.log(`  ⏳ [Human Action Delay] Pausing ${(delayMs / 1000).toFixed(1)}s before next step...`);
...
async function runFullDailyRoutine(client, options = {}) {
  const preferredBrand = client.account?.preferredBrand || "amazon";
  const targetId = client.account?.accountId || client.accountId;
  const summary = {
    streakHeal: null,
    dailyBonus: null,
    dailySpin: null,
  ...
  const isAccountDeleted = () => {
    try {
      ...
      return !fresh.some((a) => a.accountId === targetId);
    } catch (_) {
      return false;
    }
  };
  ...
  if (isAccountDeleted()) {
    console.log(`  🛑 [Account Deleted] ${targetId} was removed from Cloud DB. Aborting routine immediately.`);
    return summary;
  }
  ...
  if (isAccountDeleted()) {
    console.log(`  🛑 [Account Deleted] ${client.accountId} was removed from Cloud DB. Aborting routine immediately.`);
    return summary;
  }
