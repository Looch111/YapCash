 ...
 * Features: Auto token refresh on 401, structured outcome return, and audit logging to pack_history.json.
 */
  ...
  try {
    ...
    if (res.ok && result.ok !== false) {
      ...
      if (rewardInfo?.type === "gift_card" || rewardInfo?.brand || rewardInfo?.claimUrl || rewardInfo?.code) {
        ...
        xpBonus = rewardInfo.xp || rewardInfo.amount || 0;
        winMsg = `Pack opened (${packId}): +${xpBonus} XP Bonus!`;
      }
...
async function runFullDailyRoutine(client, options = {}) {
  ...
  // 4. Reward pack opening temporarily disabled per user configuration
  summary.packOpens = [];
