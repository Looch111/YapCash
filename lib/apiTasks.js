...
async function runFullDailyRoutine(client, options = {}) {
  ...
  for (const packId of packTypes) {
    ...
    // If account has insufficient XP, cooldown, or region lock, skip redundant higher tier pack attempts
    if (packRes.reason && (packRes.reason.includes("insufficient_xp") || packRes.reason.includes("cooldown") || packRes.reason.includes("region"))) {
      break;
    }
  }
